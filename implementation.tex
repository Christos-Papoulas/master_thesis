...
The Network File System (NFS) is configured and used in order to allow all Social Networking Engines to gain access to the same file system store. An NFS server is installed in one of the SN Engines and all the other SN Engines have an NFS client accessing the remote file system.   
...
All tuples at Memcached are inserted with the maximum key expiration time of thirty days. When a value in the SNP is updated, the memcached key will deleted as shown in the figure~\ref{fig:memcache_key_value}.
