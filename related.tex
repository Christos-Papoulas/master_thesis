...
\begin{itemize}
\item The platform identities the similarity of a given question with the others questions already posted in the system.
\item The platform maps a question with a relevant query to the repository providing to the one who asks with an appropriate response.
\item The platform can paraphrase the queries according to the user's arbitrary input in order to meet the previous objective. 
\end{itemize}
\ldots